...
import os
#%% set parameters
def make_data_splits_1(csv_dir,trial_num):
    tr_per = 0.8
    ts_per = 0.1
    va_per = 0.1
    
    the_dir = csv_dir+'/Aim_1/trial_' + str(trial_num)
    os.makedirs(the_dir)
    #%%
    compdata = csv_dir+'/compiled_data.csv'
    #imgfolder = r"C:\Users\13019\OneDrive\Desktop\Png CXR"
    ...
    #df['fileName'] = imgfolder + compdf['patientId'] + ".png" #".dcm"
    suffix = '.jpg'
    ...
    #data = df.astype({'xmin':'int','ymin':'int','xmax':'int','ymax':'int'})
    df = df.replace(0,"")
    ...
    print("finished Image Id list")
    
    
    #%% Getting number for each group
    
    total_num = [len(f_pos),len(f_neg),len(m_pos),len(m_neg)]
    ...
    # fratio = len(f_pos)/len(f_pos+f_neg)
    # mratio = len(m_pos)/len(m_pos+m_neg)
    # check_leakage(train_ids,test_ids,val_ids,compdf)
    ...
    # check_ratios(test_ids,train_ids,val_ids,fratio,mratio)
    ...
    for group in train_ids:
        ...
        #print(traincsv.head())
    name_train = os.path.join(the_dir,'train_a1_.csv')
    ...
    name_test = os.path.join(the_dir, 'test_a1_.csv')
    ...
    name_val = os.path.join(the_dir, 'val_a1_.csv')
